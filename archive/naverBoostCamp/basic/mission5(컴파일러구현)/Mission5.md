...
> js 로 구현한 컴파일러 예시
>
토크나이저는 입력 문자열을 한 글자씩 스캔하면, 충돌 위험이 적은 기호(괄호, 세미콜론 등) 을 먼저 검사하고 식별자, 키워드 까지 차례로 토큰화한다 공백은 따로 토큰화 하지않고
반환값으로는 {type, value} 형태의 토큰 배열을 반환한다

파서는 위의 토큰 배열을 받아 var -> 식별자 -> = -> new .. 순으로 재귀하향파싱을 수행해 AST 를 만든다.
각 단계에서 expect() 로 예상되는 토큰을 확인하고 처리하며 불일치시 SyntaxError 던진다

## 배운점
처음 접하는 생소한 것을 배움에 있어서 이론을 이해하고 구현에 어려움이 있다면 우선적으로 클론 코딩을 해보면 좋다는 생각을 다시한번 느낄 수 있었다.

AST 로 코드가 처리되는 과정을 보면서
코드를 작성할때 타입의 중요성을 한번 더 느끼게 되었고

모듈화를 했을때의 장점이 크게 와닿았다
특히 expect 메서드에서 느껴졌는데
parser 함수 내부에서 자주 쓰이는 만큼 expect 를 모듈화하지 않고 직접 모든 메서드에 구현을 했다면 가독성 및 오류분석 과정에서 너무 어려웠을거 같다