...
public class GlobalExceptionAdviceV3 {
    ...
    public ResponseEntity handleBusinessLogicException(BusinessLogicException e) {
        final ErrorResponseV2 response = ErrorResponseV2.of(e.getExceptionCode());
    ...
    public ErrorResponseV2 handleHttpRequestMethodNotSupportedException(
            HttpRequestMethodNotSupportedException e) {

        final ErrorResponseV2 response = ErrorResponseV2.of(HttpStatus.METHOD_NOT_ALLOWED);
    ...
    public ErrorResponseV2 handleException(Exception e) {
        log.error("# handle Exception", e);
        // TODO 애플리케이션의 에러는 에러 로그를 로그에 기록하고, 관리자에게 이메일이나 카카오 톡,
        //  슬랙 등으로 알려주는 로직이 있는게 좋습니다.

        final ErrorResponseV2 response = ErrorResponseV2.of(HttpStatus.INTERNAL_SERVER_ERROR);
